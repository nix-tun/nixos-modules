...
}@host:
{
  ...
  config = lib.mkIf (config.nix-tun.utils.containers != { }) {
    ...
    networking.firewall.interfaces."vz-container".allowedUDPPorts = [
      53
      67
      5355
    ];
    networking.firewall.interfaces."vz-container".allowedTCPPorts = [
      53
      67
      5355
    ];
    ...
    nix-tun.services.traefik = (
      lib.mkMerge (
        lib.attrsets.mapAttrsToList (name: container: {
          entrypoints = (
            lib.listToAttrs (
              lib.lists.map (v: {
          ...
          services = (
            lib.mkMerge [
              (lib.listToAttrs (
                lib.lists.map (v: {
                  ...
                  value = {
                    protocol = v.protocol;
                    router.entryPoints = [
                      "container-${name}-${v.protocol}-${builtins.toString v.hostPort}-${builtins.toString v.port}"
                    ];
              ...
              (lib.attrsets.mapAttrs' (domain-name: domain-value: {
    ...
    containers = lib.attrsets.mapAttrs (name: value: {
      ...
      extraFlags = lib.mkMerge [
        ...
        (lib.attrsets.mapAttrsToList (
          n: v: "--bind=${config.nix-tun.storage.persist.path}/containers/${name}/${n}:${n}:idmap"
        ) value.volumes)
        (lib.lists.map (
          secret: "--bind=${config.sops.secrets."${name}-${secret}".path}:/secret/${secret}:idmap"
        ) value.secrets)
      ];
      config = lib.mkMerge [
        (
          { ... }:
          {
            config = {
              ...
              systemd.tmpfiles.rules = (
                lib.attrsets.mapAttrsToList (n: v: "d ${v.mode} ${v.owner} ${v.group} -") value.volumes
              );
              # Disable Firewall for specific Ports
              networking.firewall =
                let
                  exposedTCPPorts = lib.lists.map (x: x.port) (
                    lib.lists.filter (x: x.protocol == "tcp") value.exposedPorts
                  );
                  exposedUDPPorts = lib.lists.map (x: x.port) (
                    lib.lists.filter (x: x.protocol == "udp") value.exposedPorts
                  );
                in
                {
