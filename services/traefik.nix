...
}:
{
  ...
  config = lib.mkIf config.nix-tun.services.traefik.enable {
    ...
    environment.etc."alloy/traefik-metrics.alloy".text =
      lib.mkIf
        (config.nix-tun.alloy.prometheus-host != null && config.nix-tun.services.traefik.enable_prometheus)
        ''
    ...
    networking.firewall.allowedTCPPorts = lib.attrsets.mapAttrsToList (name: value: value.port) (
      lib.attrsets.filterAttrs (
        name: value: value.protocol == "tcp" || value.protocol == "http"
    ...
    networking.firewall.allowedUDPPorts = lib.attrsets.mapAttrsToList (name: value: value.port) (
      lib.attrsets.filterAttrs (
        name: value: value.protocol == "udp"
      ) config.nix-tun.services.traefik.entrypoints
    );

    users.users.traefik.extraGroups = lib.mkIf config.nix-tun.services.traefik.enable_docker [
      "docker"
    ];
      ...
      dynamicConfigOptions = {
        http = {
          routers = lib.mkMerge [
            ...
            ) (lib.attrsets.filterAttrs (n: v: v.protocol == "http") config.nix-tun.services.traefik.services))
          ...
          services = lib.attrsets.mapAttrs (name: value: {
        ...
        tcp =
          lib.mkIf
            (
              { }
              != (lib.attrsets.filterAttrs (n: v: v.protocol == "tcp") config.nix-tun.services.traefik.services)
            )
            {
              routers = (
                lib.attrsets.mapAttrs (
        ...
        udp =
          lib.mkIf
            (
              { }
              != (lib.attrsets.filterAttrs (n: v: v.protocol == "udp") config.nix-tun.services.traefik.services)
            )
            {
              routers = (
